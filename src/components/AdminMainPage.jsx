...
export default function AdminMainPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getReports(), getInquiry(token)]).then(([reports, inquiries]) => {
      const pendingReports = (reports || []).filter(r => r.status === "PENDING").length;
      const unansweredInquiries = (inquiries || []).filter(i => !i.adminResponses).length;
    ...
    <>
      <main className={styles.main}>
        ...
        <div className={styles.cardGrid}>
          ...
          <Link to="/report" className={styles.card} style={{ position: "relative" }}>
            <img
